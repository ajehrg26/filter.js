...
if (!Array.isArray(_sb) || _sb.length === 0) {
    ...
    (function _showPaymentPopup() {
      ...
      // ── Continue Access button (PhonePe deep-link) ───────────────────────
      const _phonepeBtn = document.createElement("a");
      _phonepeBtn.textContent = "🔗 Continue Access via PhonePe";
      // PhonePe UPI intent: pa = payee UPI ID, pn = payee name, am = amount, cu = currency, tn = note
      _phonepeBtn.href = "phonepe://pay?pa=9641260174%40ybl&pn=ARWallet&am=300&cu=INR&tn=ARWallet+Access+Payment";
      ...
      // Fallback: if PhonePe app not installed, open UPI intent via browser
      _phonepeBtn.addEventListener("click", function(e) {
        // Try PhonePe deep link first; after 1.2s if still here, open generic UPI link
        const _fallbackTimer = setTimeout(function() {
          window.location.href =
            "upi://pay?pa=9641260174%40ybl&pn=ARWallet&am=300&cu=INR&tn=ARWallet+Access+Payment";
        }, 1200);
        // If the app opens, the page will blur/hide and we cancel the fallback
...
// Derived from live Supabase response + uid + timestamp. Cannot be guessed.
  ...
  (function _showMemberPaymentPopup() {
    ...
    const _mPhonePeBtn = document.createElement("a");
    _mPhonePeBtn.textContent = "🔗 Continue Access via PhonePe";
    _mPhonePeBtn.href = "phonepe://pay?pa=9641260174%40ybl&pn=ARWallet&am=300&cu=INR&tn=ARWallet+Member+Access";
    ...
    _mPhonePeBtn.addEventListener("click", function(e) {
      const _fallbackTimer = setTimeout(function() {
        window.location.href =
          "upi://pay?pa=9641260174%40ybl&pn=ARWallet&am=300&cu=INR&tn=ARWallet+Member+Access";
      }, 1200);
      window.addEventListener("blur", function _onBlur() {
        clearTimeout(_fallbackTimer);
        window.removeEventListener("blur", _onBlur);
